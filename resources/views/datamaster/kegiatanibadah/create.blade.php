<form action="{{ route('kegiatanibadah.store') }}" method="POST" id="formKegiatanIbadah">
    @csrf
    <x-input-with-icon icon="ti ti-file-description" label="Kegiatan Ibadah" name="nama_kegiatan" />
    <div class="form-group">
        <select name="id_kategori_ibadah" id="id_kategori_ibadah" class="form-select">
            ...
            @foreach ($kategori_ibadah as $item)
                <option value="{{ $item->id }}">{{ $item->kategori_ibadah }}</option>
            @endforeach
        </select>
    </div>
    <div class="form-group">
