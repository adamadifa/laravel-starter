@extends('layouts.app')
@section('titlepage', 'Edit Agenda Kegiatan')
@section('content')
    <form action="{{ route('agendakegiatan.update', ['id' => Crypt::encrypt($agenda_kegiatan->id)]) }}" id="formEditAgendakegiatan" method="POST"
        ...
        <div class="form-group mb-3">
            <textarea name="uraian_kegiatan" id="uraian_kegiatan" class="form-control" rows="10" placeholder="Uraian Kegiatan">{{ $agenda_kegiatan->uraian_kegiatan }}</textarea>
        </div>
...
@push('myscript')
    <script>
        $(function() {
            ...
            function getJobdesk() {
                let kode_jabatan = $("#formEditAgendakegiatan").find('#kode_jabatan').val();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        for (let i = 0; i < response.length; i++) {
                            $("#formEditAgendakegiatan").find("#kode_jobdesk").append('<option value="' + response[i]
            ...
            $("#formEditAgendakegiatan").find('#kode_jabatan, #kode_dept').on('change', function() {
                getJobdesk();
            });
