@extends('layouts.app')
@section('titlepage', 'Edit Realisasi Kegiatan')
@section('content')
    <form action="{{ route('realisasikegiatan.update', Crypt::encrypt($realisasikegiatan->id)) }}" id="formEditerealisasikegiatan" method="POST"
        ...
        {{-- <div class="form-group mb-3">
            <select name="kode_jabatan" id="kode_jabatan" class="form-select select2Kodejabatan">
                ...
                @foreach ($jabatan as $d)
                    <option value="{{ $d->kode_jabatan }}" {{ $realisasikegiatan->kode_jabatan == $d->kode_jabatan ? 'selected' : '' }}>{{ strtoUpper($d->nama_jabatan) }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group mb-3">
            <select name="kode_dept" id="kode_dept" class="form-select select2Kodedept">
                ...
                @foreach ($departemen as $d)
                    <option value="{{ $d->kode_dept }}" {{ $realisasikegiatan->kode_dept == $d->kode_dept ? 'selected' : '' }}>{{ strtoupper($d->nama_dept) }}</option>
                @endforeach
            </select>
        </div> --}}
        <div class="form-group mb-3">
        ...
        <div class="form-group mb-3">
            <textarea name="uraian_kegiatan" id="uraian_kegiatan" class="form-control" rows="10" placeholder="Uraian Kegiatan">{{ $realisasikegiatan->uraian_kegiatan }}</textarea>
        </div>
